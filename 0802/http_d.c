...
void *handle(void *arg)
{
    ...
    snprintf(filepath, sizeof(filepath), "/home/xuezha/c语言/0802/html%s", path);
...
/**
 * @brief 
 * 守护进程五步：fork->setsid->fork二次脱离终端->切换工作目录->重定向标准IO到空设备
 */
void daemonize(void)
{
    pid_t pid;
    // 1. 第一次fork，父进程退出，子进程脱离控制台前台
    pid = fork();
    if (pid > 0)
        exit(EXIT_SUCCESS);
    if (pid < 0)
    ...
    // 4. 切换工作目录至根目录，防止挂载点无法卸载
    chdir("/");

    // 5. 重置文件权限掩码
    umask(0);

    // 6. 关闭、重定向标准输入输出，脱离控制台
...
int main(void)
{
    ...
    // 守护进程无控制台，printf写入/dev/null，控制台看不到打印
    // 可自行替换为文件日志落地
    while (1)
    {
